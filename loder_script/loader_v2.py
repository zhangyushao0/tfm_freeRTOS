...
output_relocation_info_path = "platform/ext/target/stm/aslr_freertos_nsapp/loader/src/relocation.c"
output_functions_info_path = "platform/ext/target/stm/aslr_freertos_nsapp/loader/src/func.c"
...
def output_relocation_info(info):
    with open(output_relocation_info_path, 'w') as f:
        ...
        for entry in info:
            ...
            type = entry[2]
            name = entry[3]
            f.write('    {' + address + ', ' + value + ', ' + str(type) + '},' + '  //' + name + '\n')
        f.write("};\n")
...
def output_functions_info(functions_info):
    with open(output_functions_info_path, 'w') as f:
        ...
        for addr, size, name in functions_info:
            f.write('    {' + hex(addr) + ', ' + str(size) + ', ' + str(0) +'},' + '  //' + name + '\n')
        f.write("};\n")
...
def parse_symbol_table(elf_file):
    ...
    for symbol in symbol_tables.iter_symbols():
        table_infos.append([symbol['st_value'], symbol['st_size'], symbol["st_info"]["type"], symbol["st_shndx"], symbol.name if symbol.name else ""])
    ...
    for section in elf_file.iter_sections():
        for need_section in need_relocation_secions:
            if section.name == ".rel." + need_section:
                index = section_index("." + need_section, sections_info)
                rel_sections.append([section, index])
...
def generate_functions_info(symbol_tables, sections_info):
    ...
    for symbol in symbol_tables:
        if symbol[2] == "STT_FUNC":
            functions_info.append([symbol[0] + sections_info[symbol[3]][0], symbol[1], symbol[4]])

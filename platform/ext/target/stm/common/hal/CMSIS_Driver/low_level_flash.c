...
static const ARM_FLASH_CAPABILITIES DriverCapabilities = {
    EVENT_READY_NOT_AVAILABLE, DATA_WIDTH_32BIT, CHIP_ERASE_SUPPORTED};
 ...
 *               otherwise.
 */
static bool is_range_secure(struct arm_flash_dev_t *flash_dev, uint32_t start,
                            uint32_t len) {
  ...
  for (nb = 0; nb < vect->nb; nb++)
    if ((start >= vect->range[nb].base) &&
        ((start + len - 1) <= vect->range[nb].limit)) {
      return true;
    }
  return false;
}
 ...
 *               otherwise.
 */
static bool is_erase_allow(struct arm_flash_dev_t *flash_dev, uint32_t param) {
 ...
 *               otherwise.
 */
static bool is_write_allow(struct arm_flash_dev_t *flash_dev, uint32_t start,
                           uint32_t len) {
  /*  allow write access in area provided by device info */
 ...
 *               otherwise.
 */
static uint32_t bank_number(struct arm_flash_dev_t *flash_dev, uint32_t param) {
...
static uint32_t page_number(struct arm_flash_dev_t *flash_dev, uint32_t param) {
  ...
  page = ((page > (flash_dev->data->sector_count)))
             ? page - ((flash_dev->data->sector_count))
             : page;
...
static struct arm_flash_dev_t ARM_FLASH0_DEV = {.dev = &(FLASH0_DEV),
                                                .data = &(ARM_FLASH0_DEV_DATA)};

__attribute__((naked)) static void memcpy_flash(void *dest, const void *src,
                                                size_t n) {
  __ASM volatile(
...
static ARM_DRIVER_VERSION Flash_GetVersion(void) { return DriverVersion; }
...
static int32_t Flash_Initialize(ARM_Flash_SignalEvent_t cb_event) {
  ...
  printf("Flash operation: Op=0x%x, Area=0x%x, Address=0x%x",
         flash_operation.OperationType, flash_operation.FlashArea,
...
static int32_t Flash_Uninitialize(void) { return ARM_DRIVER_OK; }
...
#error "flash configuration must be defined here"
  ...
  do {
    ...
    if ((dword[0] != -1) || (dword[1] != -1))
      err = HAL_FLASH_Program(write_type, (flash_base + addr),
                              (uint32_t)&dword[0]);
    else
      err = HAL_OK;
#else
...
static int32_t Flash_EraseChip(void) { return ARM_DRIVER_ERROR_UNSUPPORTED; }

static ARM_FLASH_STATUS Flash_GetStatus(void) { return ARM_FLASH0_STATUS; }

static ARM_FLASH_INFO *Flash_GetInfo(void) { return ARM_FLASH0_DEV.data; }
...
/*
   As this handler code relies on stack pointer position to manipulate the PC
   return value, it is important not to use extra registers (on top of scratch
   registers), because it would change the stack pointer position. Then compiler
   optimizations are customized to ensure that.
*/
...
void NMI_Handler(void) {
  ...
  if (__HAL_FLASH_GET_FLAG(FLASH_FLAG_ECCD)) {
    ...
    /* Check EXC_RETURN value in LR to know which SP was used prior entering
     * exception */
    if (((lr) & (0xFU)) == 0xDU) {
    ...
    if ((*(p_sp + 7U) & (1U << xPSR_T_Pos)) != 0U) {
      ...
      */
      ...
      if ((opcode_msb == 0xE800U) || (opcode_msb == 0xF000U) ||
          (opcode_msb == 0xF800U)) {

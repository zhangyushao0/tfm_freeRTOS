...
#error "flash configuration must be defined here"
     ...
     */
    if ((!is_range_valid(&ARM_FLASH0_DEV, addr + cnt - 1)) || (!is_write_aligned(&ARM_FLASH0_DEV, addr)) || (!is_write_aligned(&ARM_FLASH0_DEV, cnt)) || (!is_write_allow(&ARM_FLASH0_DEV, addr, cnt))) {
        ARM_FLASH0_STATUS.error = DRIVER_STATUS_ERROR;
        return ARM_DRIVER_ERROR_PARAMETER;
    }
...
static int32_t Flash_EraseSector(uint32_t addr) {
    ...
    printf("erase %x\r\n", addr);
#endif /* DEBUG_FLASH_ACCESS */
    if (!(is_range_valid(&ARM_FLASH0_DEV, addr)) || !(is_erase_aligned(&ARM_FLASH0_DEV, addr)) || !(is_erase_allow(&ARM_FLASH0_DEV, addr))) {
        ARM_FLASH0_STATUS.error = DRIVER_STATUS_ERROR;
#ifdef DEBUG_FLASH_ACCESS

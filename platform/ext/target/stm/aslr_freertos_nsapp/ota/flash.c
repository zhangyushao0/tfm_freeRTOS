...
HAL_StatusTypeDef Flash_Write_Page(uint32_t Page, uint8_t *pBuffer) {
  ...
  for (unsigned int i = 0; i < FLASH_PAGE_SIZE; i += 8) {
    ...
    if (HAL_FLASH_Program(FLASH_TYPEPROGRAM_DOUBLEWORD, address + i, data) !=
        HAL_OK) {
      return HAL_ERROR;
      //		    while (1)
      //		    {
  ...
  for (unsigned int i = 0; i < num_page; i++) {
    if (i == 0) {
      ...
      for (unsigned int j = 0; j < FLASH_PAGE_SIZE; j += 8) {
        ...
        if (HAL_FLASH_Program(FLASH_TYPEPROGRAM_DOUBLEWORD, page_address,
                              data) == HAL_OK) {
          page_address += 8;
        } else {
          return HAL_ERROR;
...
uint32_t GetPage(uint32_t Address) {
  ...
  if (Address >= FLASH_BASE && Address < (FLASH_BASE + FLASH_BANK_SIZE)) {
    ...
    page =
        (Address - (FLASH_BASE_NS + FLASH_BANK_SIZE)) / FLASH_PAGE_SIZE + 128U;
  }

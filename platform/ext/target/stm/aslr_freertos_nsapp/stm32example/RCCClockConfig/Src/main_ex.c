...
static void SwitchSystemClock(void) {
  if (__HAL_RCC_GET_PLL_OSCSOURCE() == RCC_PLLSOURCE_HSI) {
    ...
    /* Set SYSCLK frequency to 110000000 Hz, coming from the PLL which is
     * clocked by MSI */
...
static void SystemClockHSI_Config(void) {
  ...
  /* -1- Select MSI as system clock source to allow modification of the PLL
   * configuration */
  ...
  /* -2- Enable HSI Oscillator, select it as PLL source and finally activate the
   * PLL */
  ...
  /* -3- Select the PLL as system clock source and configure the HCLK, PCLK1 and
...
static void SystemClockMSI_Config(void) {
  ...
  /* -1- Select HSI as system clock source to allow modification of the PLL
   * configuration */
  RCC_ClkInitStruct.ClockType = RCC_CLOCKTYPE_SYSCLK;
  ...
  /* -2- Enable MSI Oscillator, select it as PLL source and finally activate the
   * PLL */
  RCC_OscInitStruct.OscillatorType = RCC_OSCILLATORTYPE_MSI;
  RCC_OscInitStruct.MSIState = RCC_MSI_ON;
  RCC_OscInitStruct.MSIClockRange = RCC_MSIRANGE_6;
  ...
  /* -4- Optional: Disable HSI Oscillator (if the HSI is no more needed by the
   * application)*/
  RCC_OscInitStruct.OscillatorType = RCC_OSCILLATORTYPE_HSI;

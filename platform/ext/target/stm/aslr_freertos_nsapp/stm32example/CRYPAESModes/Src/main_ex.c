...
/* USER CODE BEGIN PV */
/* Key size 256 bits */
__ALIGN_BEGIN static const uint32_t pKeyAES256[8] __ALIGN_END = {
...
uint32_t aPlaintextCBC[AES_TEXT_SIZE] = {
    0xE2BEC16B, 0x969F402E, 0x117E3DE9, 0x2A179373, 0x578A2DAE, 0x9CAC031E,
    0xAC6FB79E, 0x518EAF45, 0x461CC830, 0x11E45CA3, 0x19C1FBE5, 0xEF520A1A,
    0x45249FF6, 0x179B4FDF, 0x7B412BAD, 0x10376CE6};

uint32_t aPlaintextCTR[AES_TEXT_SIZE] = {
    0x477D83D6, 0x69F90274, 0x887EBC97, 0x54E8C9CE, 0xEA51B475, 0x3935C078,
    0x35F6ED79, 0x8A71F5A2, 0x6238130C, 0x88273AC5, 0x9883DFA7, 0xF74A5058,
...
uint32_t aEncryptedtextECB128[AES_TEXT_SIZE] = {
    0x3AD77BB4, 0x0D7A3660, 0xA89ECAF3, 0x2466EF97, 0xF5D3D585, 0x03B9699D,
    0xE785895A, 0x96FDBAAF, 0x43B1CD7F, 0x598ECE23, 0x881B00E3, 0xED030688,
...
uint32_t aEncryptedtextECB256[AES_TEXT_SIZE] = {
    0xF3EED1BD, 0xB5D2A03C, 0x064B5A7E, 0x3DB181F8, 0x591CCB10, 0xD410ED26,
    0xDC5BA74A, 0x31362870, 0xB6ED21B9, 0x9CA6F4F9, 0xF153E7B1, 0xBEAFED1D,
...
uint32_t aEncryptedtextCBC128[AES_TEXT_SIZE] = {
    0xACAB4976, 0x46B21981, 0x9B8EE9CE, 0x7D19E912, 0x9BCB8650, 0xEE197250,
    0x3A11DB95, 0xB2787691, 0xB8D6BE73, 0x3B74C1E3, 0x9EE61671, 0x16952222,
...
uint32_t aEncryptedtextCBC256[AES_TEXT_SIZE] = {
    0x044C8CF5, 0xBAF1E5D6, 0xFBAB9E77, 0xD6FB7B5F, 0x964EFC9C, 0x8D80DB7E,
    0x7B779F67, 0x7D2C70C6, 0x6933F239, 0xCFBAD9A9, 0x63E230A5, 0x61142304,
...
uint32_t aEncryptedtextCTR128[AES_TEXT_SIZE] = {
    0x8986B2E1, 0x64C7046D, 0x2616F7D8, 0x736DB099, 0xD66F6019, 0xFFBF0E9E,
    0xDE18E861, 0xFFBFFF9D, 0x7CFB275A, 0x7ACBABDB, 0x4090F2DA, 0xD57C0DB0,
...
uint32_t aEncryptedtextCTR256[AES_TEXT_SIZE] = {
    0xC8C37806, 0xA591EAEE, 0x20AFE5ED, 0x144BCFDD, 0x53C7C22F, 0x59AD46B2,
    0x09972153, 0xA3AF5353, 0x5B0C90D4, 0x3297BC45, 0x5DE80E17, 0xB11921B4,
...
static void Display_EncryptedData(uint8_t mode, uint16_t keysize,
                                  uint32_t datalength);
static void Display_DecryptedData(uint8_t mode, uint16_t keysize,
                                  uint32_t datalength);
...
int main_ex(void) {
  ...
  /*****************  AES 128   ****************/
  /* Start encrypting aPlaintextECB, the cypher data is available in
   * aEncryptedtext */
  if (HAL_CRYP_Encrypt(&hcryp, aPlaintextECB, AES_TEXT_SIZE, aEncryptedtext,
  ...
  hcryp.Init.pKey = (uint32_t *)pKeyAES256;
  ...
  if (HAL_CRYP_Encrypt_IT(&hcryp, aPlaintextECB, AES_TEXT_SIZE,
                          aEncryptedtext) != HAL_OK) {
    Error_Handler();
  }
  ...
  /* Start decrypting aCyphertext, the decrypted data is available in
   * aDecryptedtext */
  if (HAL_CRYP_Decrypt(&hcryp, aEncryptedtextECB128, AES_TEXT_SIZE,
                       aDecryptedtext, TIMEOUT_VALUE) == HAL_OK) {
  ...
  if (HAL_CRYP_Decrypt_IT(&hcryp, aEncryptedtextECB256, AES_TEXT_SIZE,
                          aDecryptedtext) != HAL_OK) {
    Error_Handler();
  }
  /* Wait for processing to be done */
  while (HAL_CRYP_GetState(&hcryp) != HAL_CRYP_STATE_READY)
  ...
  /* Start encrypting aPlaintextECB, the cypher data is available in
   * aEncryptedtext */
  if (HAL_CRYP_Encrypt(&hcryp, aPlaintextCBC, AES_TEXT_SIZE, aEncryptedtext,
                       TIMEOUT_VALUE) == HAL_OK) {
    /* Display encrypted Data */
  ...
  if (HAL_CRYP_Encrypt_IT(&hcryp, aPlaintextCBC, AES_TEXT_SIZE,
                          aEncryptedtext) != HAL_OK) {
    Error_Handler();
  }
  /* Wait for processing to be done */
  ...
  /* Start decrypting aCyphertext, the decrypted data is available in
   * aDecryptedtext */
  if (HAL_CRYP_Decrypt(&hcryp, aEncryptedtextCBC128, AES_TEXT_SIZE,
                       aDecryptedtext, TIMEOUT_VALUE) == HAL_OK) {
    /* Display decrypted Data */
  ...
  if (HAL_CRYP_Decrypt_IT(&hcryp, aEncryptedtextCBC256, AES_TEXT_SIZE,
                          aDecryptedtext) != HAL_OK) {
    Error_Handler();
  }
  /* Wait for processing to be done */
  ...
  /* Start encrypting aPlaintextCTR, the cypher data is available in
   * aEncryptedtext */
  if (HAL_CRYP_Encrypt(&hcryp, aPlaintextCTR, AES_TEXT_SIZE, aEncryptedtext,
                       TIMEOUT_VALUE) == HAL_OK) {
    /* Display encrypted Data */
  ...
  if (HAL_CRYP_Encrypt_IT(&hcryp, aPlaintextCTR, AES_TEXT_SIZE,
                          aEncryptedtext) != HAL_OK) {
    Error_Handler();
  }
  /* Wait for processing to be done */
  ...
  /* Start decrypting aCyphertext, the decrypted data is available in
   * aDecryptedtext */
  if (HAL_CRYP_Decrypt(&hcryp, aEncryptedtextCTR128, AES_TEXT_SIZE,
                       aDecryptedtext, TIMEOUT_VALUE) == HAL_OK) {
    /* Display decrypted Data */
  ...
  if (HAL_CRYP_Decrypt_IT(&hcryp, aEncryptedtextCTR256, AES_TEXT_SIZE,
                          aDecryptedtext) != HAL_OK) {
    Error_Handler();
  }
  /* Wait for processing to be done */
 ...
 */
static void data_cmp(uint32_t *EncryptedText, uint32_t *RefText, uint8_t Size) {
  /*  Before starting a new process, you need to check the current state of the
     peripheral; if it�s busy you need to wait for the end of current transfer
     before starting a new one. For simplicity reasons, this example is just
     waiting till the end of the process, but application may perform other
     tasks while transfer operation is ongoing. */
  while (HAL_CRYP_GetState(&hcryp) != HAL_CRYP_STATE_READY) {
  }

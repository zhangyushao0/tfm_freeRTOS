#define TFM_ISOLATION_LEVEL 1
#define CONFIG_TFM_SPM_BACKEND SFN
#define TFM_PARTITION_NS_AGENT_MAILBOX 0
#define TFM_PARTITION_PROTECTED_STORAGE 0
#define TFM_PARTITION_INTERNAL_TRUSTED_STORAGE 1
#define TFM_PARTITION_CRYPTO 0
#define TFM_PARTITION_PLATFORM 0
#define TFM_PARTITION_INITIAL_ATTESTATION 0
#define TFM_PARTITION_FIRMWARE_UPDATE 0
#define TEST_FRAMEWORK_S 1
#define TFM_PARTITION_IPC_TEST 0
#define TEST_S_PS 0
#define TEST_NS_SLIH_IRQ 0
#define TEST_NS_FLIH_IRQ 0
#define TEST_PARTITION_FPU_TEST 0
#define TFM_PARTITION_SFN_BACKEND_TEST 0
/* #undef PS_STACK_SIZE */
/* #undef ITS_STACK_SIZE */
/* #undef CRYPTO_STACK_SIZE */
/* #undef PLATFORM_SP_STACK_SIZE */
/* #undef ATTEST_STACK_SIZE */
/* #undef FWU_STACK_SIZE */

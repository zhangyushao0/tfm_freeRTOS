...
int main(void) {
    ...
    uint32_t new_handler_section = 0x20017000;
    uint32_t new_trampline_section = 0x20019000;

    tramp_stack_addr = 0x2001a000; // tramp stack address

    vector_offset = vector_table.region_start - __text_address__;

    tramp_blx_addr = old_trampline_blx.region_start + new_trampline_section - old_trampline_section.region_start;

    int reset_region = loader(&a, &b, &vector_table, new_table_addr, new_trampline_section, new_handler_section, __text_address__);

    a.region_size = 0x5000;
    b.region_size = 0x5000;
